...
1. **Remove the fabricated review rating** *(Schema policy risk)* — ✅ **DONE** (commit `4ee1eb2`, live on `main`). No `aggregateRating`/`ratingValue`/`reviewCount`/`review` anywhere; `app/layout.tsx` carries a comment documenting the deliberate removal.
   - In `app/layout.tsx`, delete the `aggregateRating` block from the `ProfessionalService`/`LocalBusiness` node — **permanent removal, not "until reviews exist"** (correction 2026-07-16, see below).
   - Restoring `<Testimonials />` in `app/page.tsx:19` with genuine client reviews is still worthwhile (UX/E-E-A-T) — ⬜ **still pending: no genuine client reviews yet** (confirmed 2026-07-15). But render them **without** rating/review schema markup.
   - **Why `aggregateRating` stays out permanently:** (a) Google's review snippet policy requires first-party ratings — copying reviews from GBP/Facebook/Trustpilot into your own schema violates it; (b) since Sept 2019 Google ignores "self-serving" `aggregateRating`/`review` on `LocalBusiness`/`Organization` (entity reviewed = site owner), so even genuine first-party reviews on a `ProfessionalService` node won't produce stars in SERP. GBP reviews deliver their value in the local pack / Maps, not in on-site schema.
